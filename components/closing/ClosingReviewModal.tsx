...
interface ClosingReviewModalProps {
  ...
  outletName: string;
  cashierId?: string;
  /** Ketika true, render sebagai div inline tanpa Dialog wrapper */
  inlineMode?: boolean;
  /** Dipanggil setelah closing berhasil (berguna di inline mode) */
...
export function ClosingReviewModal({
  ...
  const handleConfirmClosing = async () => {
    ...
    try {
      ...
      // ✅ Dispatch event: kasir yang sedang buka outlet ini otomatis redirect ke picker
      // Gunakan BroadcastChannel agar kasir di tab lain juga terkunci
      const channel = new BroadcastChannel('kasir-channel');
      channel.postMessage({ type: 'OUTLET_CLOSED', outlet_id: outletId });
      channel.close();

      // ✅ Callback untuk parent (inline mode)
      onClosingSuccess?.();
      onClose();

      // Redirect hanya di mode Dialog (non-inline)
      if (!inlineMode) {
        setTimeout(() => {
    ...
    <div className="space-y-6 py-4">
          ...
          {reviewData.sales.by_channel.map((channel, idx) => (
            <div key={idx} className="flex items-center justify-between py-2 px-3 bg-slate-50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-slate-500">{channel.transactions} order</span>
              </div>
      ...
      <div className="bg-slate-100 border-2 border-slate-300 rounded-xl p-5">
        ...
        <div className="text-sm text-slate-700 space-y-1">
          <p><span className="font-bold">Produksi Berhasil:</span> {reviewData.production.success} pcs</p>
          <p><span className="font-bold">Terjual:</span> {reviewData.inventory.total_sold} pcs</p>
          <p>
            <span className="font-bold">Sisa Polos:</span>{' '}
            {reviewData.inventory.remaining_plain_standar + reviewData.inventory.remaining_plain_mini} pcs
          </p>
          <p><span className="font-bold">Sisa Jadi:</span> {reviewData.inventory.remaining_finished} pcs</p>
          <p><span className="font-bold">Gagal:</span> {reviewData.production.waste} pcs</p>
          <div className="pt-2 mt-2 border-t-2 border-slate-400">
            <p className="font-black text-base">
              {reviewData.production.success} ={' '}
              {reviewData.inventory.total_sold} +{' '}
              {reviewData.inventory.remaining_plain_standar + reviewData.inventory.remaining_plain_mini} +{' '}
              {reviewData.inventory.remaining_finished}
            </p>
  ...
  const actionsJSX = !isLoading && !alreadyClosed && reviewData ? (
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-3 pb-4 border-b border-slate-200">
          ...
          <div>
            <p className="font-black text-slate-800 text-sm">Laporan Closing · {outletName}</p>

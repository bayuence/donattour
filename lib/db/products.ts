...
export async function deleteCustomModeConfig(id: string) {
  try {
    const { error } = await supabase
      ...
      .eq("id", id);
    if (error) {
      console.error("Error deleting custom mode config:", error);
      return false;
    }
    return true;
  } catch (err) {
...
export async function upsertProduct(prod: Partial<ProductWithCategory>) {
  ...
  // ── Konversi semua field harga ke number ──────────────────────
  if (baseProd.harga_jual) baseProd.harga_jual = Number(baseProd.harga_jual);
  if (baseProd.harga_pokok_penjualan)
    baseProd.harga_pokok_penjualan = Number(baseProd.harga_pokok_penjualan);

  // ── HPP per-item: petakan ke kolom yang sudah ada di DB ───────
  // CATATAN: Kolom hpp_base_donat, hpp_topping, hpp_total, margin_amount,
  // margin_percent BELUM ADA di Supabase. Setelah menjalankan
  // migration SQL (lihat: MIGRATION_HPP_PER_ITEM.sql), hapus blok
  // "COMPATIBILITY STRIP" di bawah dan aktifkan blok "NEW COLUMNS".

  const hppBase = Number(baseProd.hpp_base_donat ?? 0);
  const hppTopping = Number(baseProd.hpp_topping ?? 0);
  const hppTotal = hppBase + hppTopping;

  // Simpan ke kolom baru yang sudah ada di DB
  baseProd.hpp_base_donat = hppBase;
  baseProd.hpp_topping = hppTopping;
  baseProd.hpp_total = hppTotal;
  // Juga simpan ke kolom lama untuk backward compat
  if (hppTotal > 0) baseProd.harga_pokok_penjualan = hppTotal;
  // Hitung margin otomatis
  if (baseProd.harga_jual && hppTotal > 0) {
    baseProd.margin_amount = baseProd.harga_jual - hppTotal;
    baseProd.margin_percent =
      baseProd.harga_jual > 0
        ? Math.round(
            (baseProd.margin_amount / baseProd.harga_jual) * 100 * 100,
          ) / 100
        : 0;
  }

  // Strip field lain yang tidak ada di DB
  ...
  try {
    ...
    // SMART UPSERT: Jika ID tidak ada, paksa cari berdasarkan Nama, Ukuran, dan Kategori
    if (!finalId && baseProd.nama && baseProd.ukuran) {
      ...
      if (existing) {
        finalId = existing.id;
        console.log(
          `[SmartUpsert] Menemukan produk existing ID: ${finalId} untuk: ${baseProd.nama}`,
        );
      }
    }
...
export async function upsertCategory(cat: Partial<ProductCategory>) {
  try {
    if (cat.id) {
      ...
      const kode = cat.kode || `CAT-${Date.now().toString().slice(-6)}`;
      const { error } = await supabase
        .from("product_categories")
        .insert({ ...cat, kode });
      if (error) {
    ...
    console.error("Exception in upsertCategory:", err);
    return false;
  }
}
...
export async function upsertPackage(pkg: Partial<ProductPackage>) {
  ...
  // ═══════════════════════════════════════════════════════════
  // REMOVED: channel_prices (multi-channel system deprecated)
  // ═══════════════════════════════════════════════════════════
...
export async function getProductBundlings(): Promise<ProductBundling[]> {
  ...
  if (error) {
    console.error("Error fetching product bundlings:", error);
    return [];
  }
  return data ?? [];
}
...
export async function upsertCustomTemplate(
  template: Partial<ProductCustomTemplate>,
) {
  try {
    if (template.id) {
      const { error } = await supabase
      ...
      const { error } = await supabase
        ...
        .insert(template);
      if (error) {
        console.error("Error inserting custom template:", error);
        return false;
      }
    }
    return true;
...
export async function getProductCustomTemplates(): Promise<
  ProductCustomTemplate[]
> {
  const { data, error } = await supabase
    .from("product_custom_templates")
  ...
  if (error) {
    console.error("Error fetching product custom templates:", error);
    return [];
  }
  return data ?? [];
}
